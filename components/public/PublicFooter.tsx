...
export function PublicFooter() {
  return (
    <footer className="border-t border-[#D8E1EE] bg-[#0F172A]">
      <div className="mx-auto grid max-w-7xl gap-12 px-4 py-14 sm:px-6 lg:grid-cols-[1.2fr_2.8fr] lg:px-8">
        <div>
          ...
          <div className="mt-6">
            <div className="rounded-xl border border-[#1E3A5F] bg-[#0D2137] p-3">
              <p className="text-xs font-semibold text-[#60A5FA] mb-2 flex items-center gap-1.5">
              ...
              <iframe
                ...
                width="100%"
                height="130"
                className="rounded-lg border border-[#1E3A5F] opacity-90"
                loading="lazy"
        ...
        <div className="grid gap-8 sm:grid-cols-4">
          {groups.map((group) => (
            <div key={group.title}>
              <h3 className="font-semibold text-white tracking-wider text-sm uppercase">{group.title}</h3>
          ...
          {/* Contact Column */}
          <div>
            <h3 className="font-semibold text-white tracking-wider text-sm uppercase">Contact</h3>
            <div className="mt-4 space-y-3 text-sm text-[#94A3B8]">
              <a href="mailto:callaudtix@gmail.com" className="flex items-center gap-2 hover:text-white transition">
                <Mail className="h-4 w-4 text-[#60A5FA] shrink-0" />
                <span className="truncate">callaudtix@gmail.com</span>
              </a>
              ...
              <a href="https://wa.me/16472574238" target="_blank" rel="noreferrer" className="flex items-center gap-2 hover:text-white transition">
                ...
                <span className="text-[#22C55E] font-medium">WhatsApp Chat</span>
              </a>
              <div className="flex items-start gap-2 border-t border-[#1E293B] pt-3 mt-3">
                <MapPin className="h-4 w-4 mt-0.5 shrink-0 text-[#60A5FA]" />
                <span className="text-xs leading-5">
                  5900 Balcones Dr Ste 13273<br />
                  Austin, TX 78731
                </span>
              </div>
            </div>
        ...
        <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div className="flex flex-wrap items-center gap-3">
              {badges.map((badge) => (
                <div key={badge} className="flex items-center gap-1.5 rounded-full border border-[#1E3A5F] bg-[#0D2137] px-3 py-1 text-xs font-semibold text-[#60A5FA]">
                  <ShieldCheck className="h-3.5 w-3.5" />
                  {badge}
                </div>
              ))}
            </div>
            <p className="text-xs text-[#64748B]">Enterprise grade security & regulatory compliance standard</p>
          </div>
        </div>
      </div>
